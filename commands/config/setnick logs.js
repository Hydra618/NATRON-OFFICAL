...
module.exports = {
    ...
    if (!args[0]) {
      let b = await db.fetch(`nicklog_${message.guild.id}`);
      ...
      if (message.guild.channels.cache.has(b)) {
        ...
        try {
            let a = await db.fetch(`nicklog_${message.guild.id}`)

            if (channel.id === a) {
                return message.channel.send("**This Channel is Already Set As Nicklog Channel!**")
            } else {
                client.guilds.cache.get(message.guild.id).channels.cache.get(channel.id).send("**Nicklogs Channel Set!**")
                db.set(`nicklog_${message.guild.id}`, channel.id)

                message.channel.send(`**Nicklog Channel Has Been Set Successfully in \`${channel.name}\`!**`)
            }
        } catch {
