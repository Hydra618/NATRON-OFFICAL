    ...
    if (!args[0]) {
      ...
      if (message.guild.channels.cache.has(b)) {
        return message.channel.send(
          `**Serverlog Channel Set In This Server Is \`${channelName.name}\`!**`
        );
      } else
        return message.channel.send(
        ...
        try {
            ...
            if (channel.id === a) {
                ...
                bot.guilds.cache.get(message.guild.id).channels.cache.get(channel.id).send("**Serverlog Channel Set!**")
                db.set(`logs_${message.guild.id}`, channel.id)

                message.channel.send(`**Serverlog Channel Has Been Set Successfully in \`${channel.name}\`!**`)
            }
        } catch {
